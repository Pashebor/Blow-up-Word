...
import com.example.wordsgame.R.raw;
...
public class WordsDatabase {
	...
	public boolean existsWord(String key) {
		...
		try {
			if(WordGame.get().isEnglish()) {
				ret = find(key, hash, new DataInputStream(WordGame.get().getResources().openRawResource(raw.dic_eng)));
			}/* else {
				ret = find(key, hash, new DataInputStream(WordGame.get().getResources().openRawResource(raw.dic)));
			}*/
		}catch(Throwable e) {
			WordGame.get().performingHandlingException(e);
		}
	...
	/*public void loadHashPortuguese() throws Exception {
		DataInputStream dt = new DataInputStream(WordGame.get().getResources().openRawResource(raw.hash)); 
	...
	public void loadHashEnglish() throws Exception {
		DataInputStream dt = new DataInputStream(WordGame.get().getResources().openRawResource(raw.hash_eng)); 
