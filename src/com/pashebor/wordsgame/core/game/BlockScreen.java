...
public class BlockScreen extends Scene{
	...
	List<BrickCol> cols = new ArrayList<BrickCol>(); 
	...
	ArrayList<Brick> selected = new ArrayList<Brick>();
	Brick lastSelected;
	...
	public BlockScreen() {
		for(int i=0; i < MAX_COLS; i++)
			cols.add(new BrickCol(i));
	}
	
	@Override
	public void initialize() {
		
		gap = WordGame.get().getDisplay().getWidth()*.10f;
		...
		WordGame.get().stopMusic();
	}
	
	
	public void restart() {
		Brick[] bricsAr = this.brics.toArray(new Brick[0]);
	...
	private void configLevel() {
		if(WordGame.get().getGameType() == GameType.NORMAL) {
			timeMinExplosiveAdd = 50000 - 1000 * WordGame.get().getLevel();
	...
	void addToCol(BrickCol col, Brick b) {
		if(col.last != null) {
	...
	private BrickCol nextCol() {
		
		BrickCol c = cols.get(getLastColAdd());
	...
	void addBric() {
		...
		if(timeExplosiveAdd > timeMinExplosiveAdd) {
			b = SceneFactory.createBrick(0, true);
			timeExplosiveAdd = timeExplosiveAdd - timeMinExplosiveAdd;
		} else {
			b = SceneFactory.createBrick(0, false);
		}
	...
	public void draw(Canvas canvas) {
		...
		if(WordGame.get().isDebug()) {
			Paint p = new Paint();
			p.setARGB(255, 110, 115, 165);
			
			for(int i = 0; i <= WordGame.get().getMetrics().heightPixels; i=i+10) {
...
//			b.setAnimation("deselecionando");
	...
	public void addPoints(int len, float x, float y, boolean bonus) {
		...
		if(score.getScore() > (WordGame.get().getLevel()*1000)) {
			score.setScore(0);
			levelUPGame();
		}
	...
	public void showPointAnimation(int points, float x, float y) {
		add(SceneFactory.createPoint(WordGame.get().getResources(), x, y, points), 2);
	}
	
	public void submitWord() {
			
			
			boolean exist = display.text.length() >= 3 && WordGame.get().getWordsDatabase().existsWord(display.text);
			
			if(canRemoveBric() && exist) {
								
				Brick sel = this.selected.get(this.selected.size()/2);
				
				addPoints(display.getBasicPoints(), sel.getX(), sel.getY(), display.isBonusWord());
				
				Brick[] brics = this.selected.toArray(new Brick[0]);
				for (Brick b : brics) {				
					cols.get(b.col).removeBrick(b);
	...
	public void explosiveRemove(Brick b) {
	...
	public void levelUPGame() {
		int level = WordGame.get().getLevel();
		
		if(level < 99) {
			level++;
			
			WordGame.get().getDictionary().setLevelPoints(level, (int) score.getScore());
			WordGame.get().setLevel(level);
		...
		if(isShowMenu()) {
			...
			try {
				...
				if(submitButton.getRectF().contains(x, y)) {
					...
					Brick[] brics = this.brics.toArray(new Brick[0]);
					for (Brick b : brics) {
