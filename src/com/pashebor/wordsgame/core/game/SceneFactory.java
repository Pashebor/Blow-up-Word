...
public class SceneFactory {
	...
	public static Typeface getSystemFont() {
		...
		if(systemFont == null)
			systemFont = Typeface.createFromAsset(WordGame.get().getResources().getAssets(), "fonts/bebasneue.ttf");
...
//		//Log.w("WORDHUNTER" , "Letra:" + letter + " T:" + ret);
	...
	public static Scene getStartScreen(Resources res) {
		...
		logo.setX(WordGame.get().getDisplay().getWidth()/2 - logo.getW()/2);
		logo.setY(WordGame.get().getDisplay().getHeight()/2 - logo.getH()/2);
	...
	public static Scene getBlockScreen(Resources res) {
		...
		DisplayMetrics met = WordGame.get().getMetrics();
		...
		sep.setX((WordGame.get().getDisplay().getWidth()-sep.getW())/2);//8
//		sep.setY(Engine.get().getDisplay().getHeight() - sep.getH());// 565
		...
		//LOADING SCORE
		scene.score.setScore(WordGame.get().getDictionary().getPoints());
...
//		Log.w("WORDHUNTER", "Engine.get().getDisplay().getHeight()::"+Engine.get().getDisplay().getHeight());
		scene.setMaxY(WordGame.get().getDisplay().getHeight()*60.98f/100);
	...
	public static CrossOut createCrossOut() {
		CrossOut sub = new CrossOut();
		
		if(explosionBmp == null)
			explosionBmp = BitmapFactory.decodeResource(WordGame.get().getResources(), R.drawable.explosion);
	...
	public static Brick createBrick(int col, boolean timer) {
		
		bricsCreated += Timer.getLast();
		
		Brick bric = new Brick("Bric"+UUID.randomUUID());
		...
		if(bricBmp == null)
			bricBmp = BitmapFactory.decodeResource(WordGame.get().getResources(), R.drawable.block_letter); 
